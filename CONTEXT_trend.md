...
> V/W/X/Y/AA/AB/AC/AD 공통 결론: 현재 채택 파라미터가 모든 변형 대비 OOS 최우수. 추가 개선 여지 미확인.
...
| 21 | **atr_risk_pct 재검증 (AE 시리즈)** | 현재 4% — bear=MA200+heat_cap 환경 최적값 미검증. 2%/3%/4%/5%/6% 스윕 | 미완료 |
| 22 | **max_positions 재검증 (AF 시리즈)** | 현재 4 — top_n=5와 비대칭. 3/4/5/6 스윕 | 미완료 |
| 23 | **OOS 구간 분할 검증 (AG 시리즈)** | OOS를 2023H2/2024/2025~현재로 분할 — 취약 구간 파악, 실전 운용에 직접 유용 | 미완료 |

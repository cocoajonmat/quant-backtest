...
**마지막 백테스트 결과 (5년, $10,000 초기자본, SPY +84.3%)**
| 전략 | 수익률 | CAGR | MDD | 샤프 |
|------|--------|------|-----|------|
| 초기 하이브리드 (실험1) | +188.4% | 29.0% | -16.8% | 1.27 |
| + 트레일링 스탑 (실험7) | +203.1% | 30.5% | -14.0% | 1.42 |
| **+ ATR 5% 사이징 (실험9, 현재 채택)** | **+434.1%** | **49.6%** | **-23.5%** | **1.39** |

현재 채택 파라미터: hybrid / bear=none / pct12 / trailing_stop=on / atr_sizing=5.0%

**다음 작업:** CONTEXT.md 실험 목록 참고
1. ~~bear_filter=block (실험3)~~ → 역효과 기각
2. ~~stop_mode=atr (실험4)~~ → 역효과 기각
3. ~~Strong80+ 임계값 (실험5A)~~ → 유니버스 부족으로 역효과 기각
4. ~~Portfolio Heat Cap (실험5B)~~ → 구조적으로 미발동 기각
5. ~~ATR 사이징 + 트레일링 스탑 튜닝 (실험6~9)~~ → 완료, ATR 5.0% 채택
6. 추가 개선 방향 미정 — 다음 세션에서 논의
